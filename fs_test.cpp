...
void readFile(fs::FS &fs, const char * path){
    ...
    Serial.printf(" - read %u bytes in %u ms\r\n", file.size(), end);
}
...
void setup(){
    ...
    delay(5000); // Give time for Serial to initialize
    if(!LittleFS.begin(FORMAT_LITTLEFS_IF_FAILED)){
        Serial.println("LittleFS Mount Failed");
        return;
    }
    Serial.println("LittleFS Mount Success");
    listDir(LittleFS, "/", 5); // List the root directory
    readFile(LittleFS, "/config_gui2.html"); // Read a file in the root directory
    readFileToString(LittleFS, "/config_gui2.html"); // Read a file in the root directory to string
    testFileIO(LittleFS, "/config_gui2.html");
